...
beam_length = 1200  # covers almost entire frame
beam_angle = math.radians(25)  # narrow cone
beam_direction = -math.pi / 2  # initially up



# Hidden objects (10, scattered far and wide)
object_files = ["assets/applee.png", "assets/candy.png", "assets/key.png", "assets/smile.png",
...
for i in range(10):
    ...
    rect = img.get_rect()
    # Scatter objects even more widely, including near edges and top
    rect.x = random.randint(0, WIDTH - rect.width)
    rect.y = random.randint(0, platform_y - 450)
    hidden_objects.append({"img": img, "rect": rect, "found": False, "name": os.path.basename(img_path)})
...
while running:
        ...
        if event.type == pygame.QUIT:
            ...
            for obj in hidden_objects:
                if not obj["found"] and obj["rect"].collidepoint(mx, my):
                    ...
                    break
            if clicked_object:
                # Only collect if inside light and torch moves are not over
                ...
                if dist < beam_length and abs(angle_diff(angle_to_obj, beam_direction)) < beam_angle / 2:
                    ...
                    if torch_moves < MAX_TORCH_MOVES:
                        if clicked_object["name"] in ["stone.png", "wood.png", "arrow.png", "bush.png"]:
                            ...
                            lives = min(lives + 1, 5)
            # If not clicking an object, allow torch direction change (if moves left)
    ...
    # Reveal objects inside cone and allow clicking to collect
    for obj in hidden_objects:
        if not obj["found"]:
    ...
    # Handle mouse click to collect object if inside light
    if pygame.mouse.get_pressed()[0]:
        mx, my = pygame.mouse.get_pos()
        for obj in hidden_objects:
            if not obj["found"] and obj["rect"].collidepoint(mx, my):
                # Check if object is inside light
                dx = obj["rect"].centerx - x
                dy = obj["rect"].centery - y
                dist = math.hypot(dx, dy)
                angle_to_obj = math.atan2(dy, dx)
                if dist < beam_length and abs(angle_diff(angle_to_obj, beam_direction)) < beam_angle / 2:
                    obj["found"] = True
                    # Only change lives if torch moves are not over
                    if torch_moves < MAX_TORCH_MOVES:
                        if obj["name"] in ["stone.png", "wood.png", "arrow.png", "bush.png"]:
                            lives -= 1
                        else:
                            lives = min(lives + 1, 5)
